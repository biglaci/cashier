...
import 'spk_knpr_ucel.dart';

import 'package:flutter/widgets.dart';

import '../idempiere_rest/idempiere_client.dart';

class MspkKnprUcel extends ChangeNotifier {
  MspkKnprUcel({
  ...
  Future getData() async {
    ...
    var mJson = await IdempiereClient().getJson(
        'models/spk_knpr_ucel',
        orderBy: ['sequence'],
        select: ['Name', 'spk_knpr_ucel_id'],
        top: 100,
        skip: 0,
        showsql: true);
    // now we have a json...
    this.data = SpkKnprUcel.fromJson(mJson);
